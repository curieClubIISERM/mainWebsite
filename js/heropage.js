...
function setLenis() {
    ...
    lenis.on('scroll', ({ scroll, limit }) => {
        const progress = (scroll / limit).toFixed(2);
...
function createSpanForHoverAnimation(elementID) {
    ...
    gsap.set(secondSpans, { y: "0.9vw" });

    element.addEventListener("mouseenter", () => {
        // Animate first spans
        gsap.to(firstSpans, { y: "-0.9vw", ease: "steps(12)", stagger: 0.01 });
    ...
    element.addEventListener("mouseleave", () => {
        ...
        gsap.to(secondSpans, { y: "0.9vw", ease: "steps(12)", stagger: 0.01 });
    });
}
